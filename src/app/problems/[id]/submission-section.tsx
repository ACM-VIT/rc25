...
import type { EvalEnum, Submission } from "@prisma/client";
...
const SubmissionSection: React.FC<SubmissionSectionProps> = ({
    ...
    const renderSubmission = (
        ...
        const passedCount = submission.testcasespassed?.filter(Boolean).length ?? 0;
        const totalTests = submission.testcasespassed?.length ?? 0;
        ...
        return (
            ...
            >
                <div className="flex flex-col items-start">
                    <span className={`font-bold ${formula1Bold.className}`}>{submission.user?.name ?? "Anonymous"}</span>
                </div>
                            ...
                            <div key={submission.id}>
                                {renderSubmission(
                                    ...
                                    submissions.findIndex(
                                        (s) =>
                                            (s.testcasespassed?.filter(Boolean).length ?? 0) ===
                                            Math.max(
                                                ...submissions.map(
                                                    (sub) =>
                                                        sub.testcasespassed?.filter(Boolean).length ?? 0
                                                )
                                            )
                                    )
                                )}
