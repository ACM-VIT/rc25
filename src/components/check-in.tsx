...
import type { Team, TeamRound, UniReg, User } from "@prisma/client";
...
function CheckIn({
  ...
  const [checkedIn, setCheckedIn] = useState<string[]>(
    team.TeamRound.length > 0
      ? team.members.map((i) => i.name?.slice(-9) ?? "")
      : [],
  );
  ...
  function lookup() {
    ...
    startTransition(async () => {
      ...
      setExtra((prev) => [...prev, res.user as UserWithTeam]);
      setTimeout(() => {
        if (res.uniReg)
  ...
  function removeParticipant(userId: string) {
    startTransition(async () => {
      ...
      setCheckedIn((prev) =>
        prev.filter((i) => i !== (user.name?.slice(-9) ?? "")),
      );
      setExtraUniReg((prev) => {
  ...
  function checkInTeam() {
    startTransition(async () => {
      if (
        team.members.length <
        Number.parseInt(process.env.MIN_TEAM_CAPACITY || "1")
      ) {
        toast({
