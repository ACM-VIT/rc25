          ...
          <div className="flex flex-col w-full md:w-[90%] lg:w-[80%] xl:w-[90%] h-full">
            <DashboardBox className="flex flex-col h-[80%] p-6 mb-4 text-white">
              ...
              <div className="flex flex-row border-b-2 py-4 w-full">
                ...
                <h1 className="w-1/5 text-xs md:text-sm font-bold text-center">Difficulty</h1>
                <h1 className="w-1/5 text-xs md:text-sm font-bold text-center">Status</h1>
              </div>
              <ScrollArea className="h-[55vh] rounded-md">
                <div className="space-y-0">
                  {submissions.map((sub, index) => (
                    <div key={sub.id} className="flex flex-row border-b border-gray-700 py-4">
                      <p className="w-1/5 text-center p-2 py-0">{index + 1}</p>
                      <p className="w-1/5 text-center p-2 py-0">
                        {new Date(sub.createdAt).toLocaleTimeString()}
                      </p>
                      {/* <p className="w-1/6 text-center p-2 py-0">{sub.user.name}</p> */}
                      <p className="w-1/5 text-center p-2 py-0">{sub.problem.title}</p>
                      <p className="w-1/5 text-center p-2 py-0">{sub.problem.difficulty}</p>
                      <p className="w-1/5 text-center p-2 py-0">
