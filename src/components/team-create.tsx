...
export default function TeamBuilder({ onTeamCreated }: TeamBuilderProps) {
  ...
  const MIN_TEAM_CAPACITY = Number(process.env.MIN_TEAM_CAPACITY || "1");
  ...
  const handleCreateTeam = async () => {
    ...
    if (selectedUsers.length < MIN_TEAM_CAPACITY) {
      toast({
        description: "Please select at least 1 user for a team",
        variant: "destructive",
      });
      return;
    }
