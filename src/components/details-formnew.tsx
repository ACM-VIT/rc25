...
<div className="w-[80%] lg:w-[70%] mb-6 text-center relative flex flex-col items-center">
  ...
  <div className="flex flex-row items-center w-full">
    ...
    <div className="w-2/3 h-[10px] bg-transparent border border-[#CEB7FF]" 
         style={{
          ...
          <div className="w-full max-w-xl px-4">
            ...
            <div className="mb-5">
              ...
              <input
                ...
                value={phone}
              />
              <div className="text-red-600 text-xs mt-0.5">{phoneError}&nbsp;</div>
            </div>

            {/* Gender Selection */}
           {/* Gender Selection (Ensures correct form submission) */}
<div className="mb-5">
  <label className={`${pt_sans.className} font-semibold text-xs text-white block mb-1`}>
    GENDER
  </label>
      ...
      <label key={gender} className="flex-1 cursor-pointer">
        ...
        <div
          className={`p-3 text-center border border-[#F0F1FA] text-lg font-semibold
                      ${selectedGender === gender ? "bg-[#CEB7FF] bg-opacity-9 text-white" : "bg-[#CEB7FF] bg-opacity-20 text-white"}`}
        >
          {gender.toUpperCase()}
        </div>
      </label>
  ...
  <div className="text-red-600 text-xs mt-0.5">{genderError}&nbsp;</div>
            ...
            {/* Status Selection */}
            <div className="mb-5">
              <label className={`${pt_sans.className} font-semibold text-xs text-white block mb-1`}>SELECT AN OPTION</label>
              <div className="flex flex-col sm:flex-row gap-4">
                {["HOSTELLER", "DAY SCHOLAR"].map((status) => (
                  <button
                    ...
                    className={`flex-1 p-3 text-center border border-[#F0F1FA]  text-lg font-semibold
                               ${selectStatus === status ? "bg-[#CEB7FF] bg-opacity-9.5 text-white" : "bg-[#CEB7FF] bg-opacity-20 text-white"}`}
...
function SubmitButton() {
    ...
    return (
      ...
      type="submit"
            ...
            disabled={pending}
      className={`px-8 py-3 bg-[#9B51E0] text-white text-xl  hover:bg-[#a765e0] ${outfit.className}`}  style={{
        color: 'transparent',
